...
const ReceivablePage = () => {
  ...
  const fetchReceivables = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await authService.getReceivables(currentUser.id);
      setReceivables(data);
    } catch (err) {
  ...
  const handleAddReceivable = async (receivableData) => {
    if (
      !authService ||
      !authService.addReceivable ||
      !currentUser ||
      !currentUser.id
    ) {
      toast.error(
        "Layanan untuk menambahkan piutang tidak tersedia atau user ID tidak ada."
      );
      throw new Error(
        "AuthService not ready or User ID missing for addReceivable."
      );
    }
    try {
      await authService.addReceivable({
        ...receivableData,
      ...
      {isFormOpen && (
        <Modal onClose={closeForm}>
          <TransactionForm
            ...
            onSubmit={
              editingReceivable ? handleUpdateReceivable : handleAddReceivable
            }
            onClose={closeForm}
          />
        </Modal>
